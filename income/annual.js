...
const insertAnnualReport = async (report, symbol) => {
  const query = `
  ...
  try {
    ...
    console.log(`Annual report for ${symbol} (${report.fiscalDateEnding}) successfully inserted`);
  } catch (err) {
    console.error(`Error inserting annual report for ${symbol} (${report.fiscalDateEnding})`, err);
  }
};
